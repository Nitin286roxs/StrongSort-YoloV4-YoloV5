...
class LoadImagesV4:  # for inference
    def __init__(self, polygon_roi, path, img_size=640, auto_size=32):
    ...
    def __next__(self):
        ...
        #cv2.imwrite("/home/nitin/Safepro/Workspace/TyreCounting/Fixing_import_issue/Yolov5_StrongSORT_OSNet/temp0.jpg", background)
        # Padded resize
        img = letterbox(background, new_shape=self.img_size, auto_size=self.auto_size)[0]
        #cv2.imwrite("/home/nitin/Safepro/Workspace/TyreCounting/Fixing_import_issue/Yolov5_StrongSORT_OSNet/temp.jpg", img)
        # Convert
...
class LoadStreamsV4:  # multiple IP or RTSP cameras
    def __init__(self, polygon_roi, sources='streams.txt', img_size=640):
        self.mode = 'images'
        self.img_size = img_size
    ...
    def __next__(self):
        ...
        '''
        #img0 = self.imgs.copy()
        print(f"shape of img0: {(img0[0].shape)}")
        backgrounds = [np.full((x.shape[0], x.shape[1], 3) , (114,114,114), np.uint8) for x in img0 ]
        print(f"self.roi: {self.roi}")
        #for x in img0:
        #    print(f"x: {x.shape}")
        #    print(f"x: {x}")
        overlays = [ x[self.roi[1]:self.roi[3], self.roi[0]:self.roi[2]] for x in img0]
        for index in range(len(overlays)):
            backgrounds[index][self.roi[1]:self.roi[3], self.roi[0]:self.roi[2]] = overlays[index]
        # Letterbox
        img = [letterbox(x, new_shape=self.img_size, auto=self.rect)[0] for x in backgrounds] 
