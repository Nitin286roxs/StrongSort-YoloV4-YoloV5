...
from yolov4.utils.google_utils import gsutil_getsize
from yolov4.utils.metrics import fitness, fitness_p, fitness_r, fitness_ap50, fitness_ap, fitness_f   
from yolov4.utils.torch_utils import init_torch_seeds
...
def check_img_sizeV4(img_size, s=32):
...
def non_max_suppressionV4(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    print(f"prediction: {prediction[0].shape}")
    ...
    t = time.time()
    output = [torch.zeros(0, 6)] * prediction[0].shape[0]
